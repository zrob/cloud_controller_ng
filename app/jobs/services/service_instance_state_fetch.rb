  ...
  module Jobs
    module Services
      class ServiceInstanceStateFetch < VCAP::CloudController::Jobs::CCJob
        attr_accessor :name, :client_attrs, :service_instance_guid, :request_attrs, :poll_interval, :end_timestamp

        def initialize(name, client_attrs, service_instance_guid, user_info, request_attrs, end_timestamp=nil)
          @name                  = name
        ...
        def get_repository
          user = User.find(guid: @user_info.guid)
          if user
            Repositories::ServiceEventRepository.new(@user_info)
          end
        end
        ...
        def record_event(service_instance, request_attrs)
          services_event_repository = get_repository
