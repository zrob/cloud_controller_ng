...
module VCAP::CloudController
  class AppStart
    ...
    class << self
      def start(app:, user_info:, record_event: true)
      ...
      def start_without_event(app)
        start(app: app, user_info: nil, record_event: false)
      end

      private

      def record_audit_event(app, user_info)
        Repositories::AppEventRepository.new(user_info).record_app_start(app)
      end
    end
  end
end
