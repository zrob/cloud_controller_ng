...
module VCAP::CloudController
  class PackageCopy
    ...
    def copy(destination_app_guid:, source_package:, record_event: true)
      ...
      package.db.transaction do
        ...
        record_audit_event(package, source_package) if record_event
      end
    ...
    def copy_without_event(destination_app_guid, source_package)
      copy(destination_app_guid: destination_app_guid, source_package: source_package, record_event: false)
    end

    private

    def record_audit_event(package, source_package)
      Repositories::PackageEventRepository.new(@user_info).record_app_package_copy(
        package,
        source_package.guid,
      )
    end
