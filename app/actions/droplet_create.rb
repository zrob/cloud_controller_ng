...
module VCAP::CloudController
  class DropletCreate
    ...
    def initialize(user_info, memory_limit_calculator=StagingMemoryCalculator.new,
    ...
    def create_and_stage(package:, lifecycle:, message:, start_after_staging: false, record_event: true)
      ...
      DropletModel.db.transaction do
        ...
        record_audit_event(droplet, message, package) if record_event
      end
    ...
    def create_and_stage_without_event(package:, lifecycle:, message:, start_after_staging: false)
      create_and_stage(package: package, lifecycle: lifecycle, message: message, start_after_staging: start_after_staging, record_event: false)
    end

    private

    def record_audit_event(droplet, message, package)
      Repositories::DropletEventRepository.new(@user_info).record_create_by_staging(
        droplet,
        message.audit_hash,
        package.app.name,
