  ...
  class Scheduler
    ...
    def start
      ...
      Locking.db.transaction do
        ...
        @clock.schedule_frequent_job(:diego_sync, Jobs::Diego::Sync, queue: 'bulk-queue', priority: -10, extra_options: {
          if: lambda { |_| Delayed::Job.where(queue: 'bulk-queue').exclude(locked_at: nil).count == 0 }
        })
