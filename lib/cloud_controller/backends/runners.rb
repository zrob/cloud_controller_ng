...
module VCAP::CloudController
  class Runners
    ...
    def dea_apps(batch_size, last_id)
      query = ProcessModel.select_all(ProcessModel.table_name).
              dea.
              where("#{ProcessModel.table_name}.id > ?", last_id).
              order("#{ProcessModel.table_name}__id".to_sym).
              limit(batch_size)
    ...
    def dea_apps_hm9k
      ...
      process_query = ProcessModel.db["Select p.id, p.app_guid, p.instances, p.version, apps.droplet_guid from processes p
      ...
      droplets_query = ProcessModel.db["select d.id, d.guid, d.app_guid, d.created_at, d.package_guid, d.state from droplets d
      ...
      packages_query = ProcessModel.db["select pkg.id, pkg.guid, pkg.app_guid, pkg.created_at, pkg.state from packages pkg
