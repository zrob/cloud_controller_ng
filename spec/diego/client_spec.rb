...
module Diego
  RSpec.describe Client do
    ...
    describe '#desired_lrp_scheduling_infos' do
      let(:lrp) { ::Diego::Bbs::Models::DesiredLRPSchedulingInfo.new }
      let(:response_body) { Bbs::Models::DesiredLRPSchedulingInfosResponse.new(error: nil, desired_lrp_scheduling_infos: [lrp]).encode.to_s }
      let(:response_status) { 200 }
      ...
      it 'returns a DesiredLRPSchedulingInfos Response' do
        ...
        expect(response.error).to be_nil
        expect(response.desired_lrp_scheduling_infos).to eq([lrp])
      ...
      context 'when the request cannot be encoded' do
        it 'raises' do
          expect { client.desired_lrp_scheduling_infos(4) }.to raise_error(EncodeError)
        end
      end
    end
