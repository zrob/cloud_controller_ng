...
module VCAP::CloudController
  module Repositories
    RSpec.describe SpaceEventRepository do
      subject(:space_event_repository) { SpaceEventRepository.new(user_info) }
      ...
      describe '#record_space_create' do
        it 'records event correctly' do
          event = space_event_repository.record_space_create(space, request_attrs)
          event.reload
      ...
      describe '#record_space_update' do
        it 'records event correctly' do
          event = space_event_repository.record_space_update(space, request_attrs)
          event.reload
          expect(event.space).to eq(space)
      ...
      describe '#record_space_delete' do
        ...
        it 'records event correctly' do
          event = space_event_repository.record_space_delete_request(space, recursive)
          event.reload
